...
college_recent_grads %>%
  arrange(desc(women)) %>%
  select(rank, major, total, women) %>%
  mutate(women = percent(women)) %>%
  top_n(3)
```
...
`The median is the middle value, so you can expect a more common outcome. It's less likely to take extreme outliers into account.`
...
`The histogram has a right skew. A majority of the data is located to the left of the graph. We can see that the mean and the median are a lot closer to the minimum than the maximum, suggesting that the data is clustered on the lower end of the distribution. There is also a clear outlier at the maximum, which can explain why the range is so large.`
...
```{r typical-median, eval=TRUE}
college_recent_grads %>%
  group_by(major_category) %>%
  summarise(typical_median =  median(median)) %>%
...
```{r filter-stem-salaries, eval = TRUE}
college_recent_grads %>%
  filter(
    major_type == "stem",
    median < 36000
...
`We can see a clear decrease here. There are fewer women in stem fields, but they are getting paid more. There is a higher proportion of women in non-stem fields, but the median income is less.`

## Further exploration

10. Ask a question of interest to you, and answer it using summary statistic(s) and/or visualization(s).

`How does unemployment vary among stem and non-stem majors?`

```{r summary-stem, eval = TRUE}
college_recent_grads %>%
  summarise(unemployed, unemployment_rate, major_type)
```
